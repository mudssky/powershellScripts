...
- 创建 `docs/INSTALL.md` 作为跨平台安装总入口，串联平台安装和 PowerShell 层安装
- 更新 `README.md` 引用 `docs/INSTALL.md`
...
### 5. 跨平台安装文档（docs/INSTALL.md）

`docs/INSTALL.md` 作为安装总入口，采用三段式结构：
...
**理由**: 总入口让 agent 从任何平台开始都能找到完整路径。平台文档负责"从零到 pwsh 可用"，跨平台文档负责"pwsh 可用后的统一步骤"。每个步骤使用与平台 INSTALL.md 相同的字段格式，保持一致性。

### 6. install.ps1 路径更新

`install.ps1` 第 355 行引用了 `linux/01manage-shell-snippet.sh`，需更新为 `shell/deploy.sh`（依赖 `shell-config-restructure` 已完成）。

### 7. README.md 引用

在根目录 `README.md` 的适当位置添加安装指南引用：

```markdown
## 安装

完整安装指南请参考 [docs/INSTALL.md](docs/INSTALL.md)。
```
