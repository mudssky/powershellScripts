...
macOS 的 `00`～`11` 与 `99` 真实叶子已接入；Linux/WSL 与 Windows 仍由各平台任务逐步接入。声明支持但入口缺失时返回 Blocked，不会回退旧脚本伪报完整成功。
...
该入口迁移期内继续调用 legacy 应用安装器并输出弃用提示，不映射为 Full，也不能与 Preset 或步骤参数组合。macOS 新机应使用 `-Preset Core|Full`。
