...
Describe 'Switch-Mirrors.ps1' {
    It '测试镜像探活应返回 Success 为 true（官方仓库）' {
        ...
        Mock -CommandName Import-SwitchMirrorsDockerAdapter -MockWith {}
        Mock -CommandName Invoke-WebRequest -ModuleName DockerAdapter -MockWith { [pscustomobject]@{ StatusCode = 200 } }
    ...
    It '测试不可达地址返回 Success 为 false' {
        ...
        Import-SwitchMirrorsDockerAdapter
        Mock -CommandName Import-SwitchMirrorsDockerAdapter -MockWith {}
        Mock -CommandName Invoke-WebRequest -ModuleName DockerAdapter -MockWith { throw 'mock unreachable' }
