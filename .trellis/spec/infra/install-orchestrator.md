...
- Text 模式必须为每个实际启动的叶子即时且仅一次向 stderr 输出 `[Running] <number> <id>: <safe-command>`。命令只能来自 `Format-InstallCommand`，不得转发任意叶子 stdout/stderr，也不得输出周期性 elapsed heartbeat。
...
理由：250ms 轮询仅让 PowerShell 能处理 Ctrl+C，不是安装超时或用户可见进度；`Kill(true)` 防止叶子退出后留下 fnm 等下载进程。
