...
- `pnpm test:pwsh:coverage`
  ...
  - 作为 `archive/openspec/specs/pester-coverage-50/spec.md` 对应的 documented coverage-enabled command。
...
- [x] coverage threshold 的 source of truth 已被澄清，`archive/openspec/specs/pester-coverage-50/spec.md` 中的规范值与实际命令输出口径不再漂移。
...
- **风险：`test:pwsh:coverage` 只是换名，没有真正成为唯一 coverage 门槛入口。**
  - 缓解：把 `archive/openspec/specs/pester-coverage-50/spec.md` 与文档都绑定到新命令，移除旧表述。
