...
Describe "Get-GpuInfo 函数测试" {
    Context "Linux 环境下 nvidia-smi CSV 输出" {
        It "应该解析 nvidia-smi CSV 输出并返回正确的 GPU 信息" {
            Mock -ModuleName hardware Get-HardwareOperatingSystem { return "Linux" }
    ...
    Context "未知操作系统" {
        It "应该返回 HasGpu=false 和 Unknown 类型" {
            Mock -ModuleName hardware Get-HardwareOperatingSystem { return "FreeBSD" }
    ...
    Context "异常处理" {
        It "应该在 Get-HardwareOperatingSystem 抛出异常时返回安全值" {
            Mock -ModuleName hardware Get-HardwareOperatingSystem { throw "OS detection failed" }
    ...
    Context "macOS 环境下无 NVIDIA GPU" {
        It "应该在无 nvidia-smi 时返回 HasGpu=false" {
            Mock -ModuleName hardware Get-HardwareOperatingSystem { return "macOS" }
            Mock -ModuleName hardware nvidia-smi { throw "command not found" }
...
Describe "Get-SystemMemoryInfo 函数测试" {
    ...
    Context "未知操作系统" {
        It "应该返回 TotalGB=0 和 AvailableGB=0" {
            Mock -ModuleName hardware Get-HardwareOperatingSystem { return "UnknownOS" }
