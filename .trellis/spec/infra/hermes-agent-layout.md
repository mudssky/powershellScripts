...
- Scope: 程序本体由官方安装目录管理，用户配置和本地状态通过 `HERMES_HOME` 指向数据盘 `/Volumes/Data/agents/hermes`。
- Design intent: 允许本地 Hermes 配置从仓库工作区分离，并用独立本地 git 仓库保存私人配置历史，同时避免把 venv、上游源码、私人配置和运行状态提交进主仓库。
...
/Volumes/Data/agents/hermes/                    # HERMES_HOME，本地私有 git 仓库
ai/agents/hermes -> /Volumes/Data/agents/hermes # 兼容旧路径的符号链接
```
...
- 主仓库必须忽略整个 `ai/agents/hermes/`；该路径只允许作为指向 `/Volumes/Data/agents/hermes` 的符号链接存在，不要把它作为 submodule 或普通目录提交。
- `/Volumes/Data/agents/hermes/` 可以初始化为独立的本地私有 git 仓库，用于记录 `config.yaml`、`.env`、`SOUL.md`、个人 `skills/` 和 `cron/` 的本地历史。
...
| `HERMES_HOME` 指向仓库真实目录且未显式分离程序目录 | 官方 installer 可能把程序装进 `$HERMES_HOME/hermes-agent`，应避免长期保留 |
| `hermes dump` | `Project` 应为 `~/.hermes/hermes-agent`，`hermes_home` 应为 `/Volumes/Data/agents/hermes` |
...
| `git check-ignore -v ai/agents/hermes` | 应命中主仓库 `.gitignore` 中的 `/ai/agents/hermes` 规则 |
| `git -C /Volumes/Data/agents/hermes status --short` | 显示 Hermes 私有仓库自己的变更，且数据库、日志、缓存、会话等运行产物不应进入 staged files |

### 5. Good/Base/Bad Cases

- Good: 程序安装在 `~/.hermes/hermes-agent`，`HERMES_HOME` 指向 `/Volumes/Data/agents/hermes`，主仓库内 `ai/agents/hermes` 只是兼容旧路径的符号链接。
- Good: `/Volumes/Data/agents/hermes` 自己是本地私有 git 仓库，提交 `config.yaml`、`.env`、`SOUL.md`、个人 skills 和 cron，忽略数据库、日志、缓存、会话、记忆和认证 token。
...
hermes --help
hermes version
HERMES_HOME=/Volumes/Data/agents/hermes hermes dump
```
...
```bash
HERMES_HOME=/Volumes/Data/agents/hermes hermes gateway status
...
```bash
git check-ignore -v ai/agents/hermes
git status --short -uall -- ai/agents/hermes
git -C /Volumes/Data/agents/hermes status --short --ignored
```
...
/Volumes/Data/agents/hermes/       # HERMES_HOME，本地私有 git 仓库
ai/agents/hermes -> /Volumes/Data/agents/hermes
```

理由：程序安装目录遵循官方布局，主仓库只保留兼容旧路径的符号链接；`/Volumes/Data/agents/hermes` 作为本机 Hermes home 和私有 git 仓库，独立管理私人配置历史。
