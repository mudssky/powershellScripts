<#
.SYNOPSIS
    安装 Windows Core/Full 共用的 Nerd Fonts。
...
if (-not $WhatIfPreference -and ($platform.SupportLevel -ne 'Full' -or $platform.IsServer)) {
    [Console]::Error.WriteLine("当前平台不支持字体自动安装: $($platform.Edition)/$($platform.Architecture)")
    exit 10
}
$catalog = Import-WindowsPackageCatalog -Path (Join-Path $repoRoot 'config/install/windows-packages.psd1')
$results = @(Install-WindowsScoopFonts -Catalog $catalog -Preview:$WhatIfPreference)
