...
4. 默认采用目录化 env/dotenv 配置，例如 `http/env/dev.env.example`、`http/env/test.env.example`、`http/env/local.env`。
...
- 默认可提交：`*.env.example`，只放 `baseUrl`、非敏感账号名、公共租户、超时和开关。
- 默认不提交：`local.env`、`*.local.env`、`.env.local` 或团队约定的私有 env 文件，放密码、token、client secret。
