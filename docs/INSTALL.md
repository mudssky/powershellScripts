...
- **Windows**: 参考 [windows/INSTALL.md](../windows/INSTALL.md)
...
macOS、Linux/WSL 与 Windows 的真实叶子均已接入。Windows 11 22H2+ x64 和 Windows 10 22H2 x64 支持 Core/Full；Windows ARM64 和 Server 返回明确 Partial/Blocked。Linux 首期完整支持 Ubuntu/Debian、WSL 客体与 amd64，Arch/ARM 返回明确 Blocked。声明支持但入口缺失时不会回退旧脚本伪报完整成功。
...
该入口迁移期内输出弃用提示，不映射为 Full，也不能与 Preset 或步骤参数组合。Linux 兼容分支调用新的 Core CLI 叶子；新机应直接使用 `-Preset Core|Full`。
