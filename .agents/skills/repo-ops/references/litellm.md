...
- `claw-plan`：OpenAI 兼容 agent 推荐稳定入口，文本优先走智谱 `GLM-5.1`，文本兜底到 NewAPI 中转 `newapi/mimo-v2.5-pro`，图片请求由 callback 切到 NewAPI 中转 `newapi/mimo-v2.5`。
- `newapi/mimo-v2.5-pro`：NewAPI 中转的小米文本兜底模型，不声明视觉能力。
- `newapi/mimo-v2.5`：NewAPI 中转的小米视觉模型，声明 `supports_vision: true`。
- `mimo-v2.5-pro` / `mimo-v2.5`：直连小米排查入口，便于和 NewAPI 中转行为对比。
...
- 小米 Mimo 直连：使用 `MIMO_API_BASE` / `MIMO_API_KEY`，模型写 `openai/mimo-v2.5-pro` 或 `openai/mimo-v2.5`，只作为手动排查入口，不作为 `claw-plan` 默认兜底。
...
- `newapi/mimo-v2.5` 是当前 claw 默认视觉目标，必须声明 `supports_vision: true`。
- `newapi/mimo-v2.5-pro` 是文本兜底模型，不声明视觉能力；如果图片请求落到它并返回 `No endpoints found that support image input`，说明视觉目标配错了。
- 直连 `mimo-v2.5` 可保留 `supports_vision: true`，用于和 NewAPI 中转视觉行为做对比。
...
- `claw-plan -> newapi/mimo-v2.5-pro` 只处理文本 GLM 失败后的兜底，不处理图片模态选择。
- `claw-plan` 图片请求由 `ClawVisionRouterAdapter` 在 `async_pre_call_hook` 阶段改写为 `newapi/mimo-v2.5`，不要期待先撞 GLM 再 fallback。
...
真实请求依赖 Docker、上游密钥、额度和网络；本地 YAML 解析通过不等价于上游可用。视觉 smoke test 优先用 base64 data URL；如果 `newapi/mimo-v2.5` 与直连 `mimo-v2.5` 行为不同，分别排查 NewAPI 渠道能力与小米端模型能力。
