...
2. 根据上下文确认目标 instance、database、schema/collection、环境、只读状态、允许动作和密钥状态。用户没有说清目标时，先不要查询。
...
- `doctor`：检查 `psql`、`mysql`、`sqlite3`、`mongosh`、`redis-cli` 等工具是否可用，只给安装提示，不自动安装。
- `exec`：执行受控动作。SQL 会自动 guard；MongoDB/Redis/Milvus 只允许配置中的只读动作。
- `client`：凭据桥接。解析配置并启动底层官方 CLI，`--` 后参数透传；它不替代 SQL guard。

`exec --verbose` 会在执行前打印脱敏执行计划；`exec --print-command` 只打印计划不执行。`client --print-command` 只打印底层客户端启动计划。
...
JSON 配置使用 `"${env:NAME}"` 引用环境变量；JS/MJS 配置可以直接使用 `process.env.NAME` 或模板字符串。密钥可以直接写在本机私有 `database-query.local.*` 中，但真实 local 文件必须被 `.gitignore` 忽略，不要提交。
