...
def _without_thinking_blocks(content: Any) -> Any:
    ...
    return [
        ...
        if not (isinstance(block, dict) and block.get("type") in THINKING_BLOCK_TYPES)
    ]
...
def _sanitize_request_data(data: dict[str, Any]) -> dict[str, Any]:
    ...
    data["messages"] = _sanitize_messages(data.get("messages"))
    return data
...
class DeepSeekThinkingSanitizer(CustomLogger):
    ...
    async def async_pre_call_deployment_hook(
        ...
        if not _is_deepseek_anthropic_request(kwargs):
            return kwargs
        return _sanitize_request_data(kwargs)
