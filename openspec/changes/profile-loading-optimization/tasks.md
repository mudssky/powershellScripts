...
- [ ] 3.1 在 `profile/features/environment.ps1` 的 `Initialize-Environment` 中，将工具初始化循环前的逐个 `Test-EXEProgram` 替换为单次 `Get-Command -Name @('starship','zoxide','sccache','fnm') -CommandType Application` 批量查询
- [ ] 3.2 将批量查询结果存入 HashSet，后续用 `$availableTools.Contains($name)` 替代 `Test-EXEProgram` 调用
- [ ] 3.3 保留工具未安装时的提示逻辑不变

## 4. 代理探测优化

- [ ] 4.1 在 `psutils/modules/proxy.psm1` 的 `Set-Proxy auto` 中将 TCP 超时从 100ms 缩短为 50ms
