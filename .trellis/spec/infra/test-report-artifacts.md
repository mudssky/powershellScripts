...
- Pester NUnit override：`PESTER_RESULT_PATH=<custom xml path>`。
- Pester coverage override：`PESTER_COVERAGE_PATH=<custom xml path>`。
...
- `PESTER_RESULT_PATH` 非空时覆盖默认 NUnit 路径；`PESTER_COVERAGE_PATH` 非空时覆盖默认 coverage 路径。
...
| 设置 `PESTER_RESULT_PATH` / `PESTER_COVERAGE_PATH` | NUnit / coverage 分别写入显式路径 |
...
- Base：调用方通过 `PESTER_RESULT_PATH` 与 `PESTER_COVERAGE_PATH` 将单次 Pester 报告隔离到唯一目录。
...
- Override test：分别设置 `PESTER_RESULT_PATH` 与 `PESTER_COVERAGE_PATH`，断言配置保留显式路径。
